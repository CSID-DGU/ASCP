...
class CrewPairingEnv(gym.Env):
    ...
    def step(self, before_idx, after_idx, pairing_set):
        ...
        # 바꾸기 전 cost 계산
        reshaped_matrix = reshape_list(pairing_set, num_rows, num_columns)  # 2차원 배열로 변환 하고
        ...
        src_last_hard, src_last_soft = calculateScore(reshaped_matrix[before_pairing_idx])
        # 페어링 인덱스를 가지고 after pairing의 cost 계산
        trg_last_hard, trg_last_soft = calculateScore(reshaped_matrix[after_pairing_idx])
        ...
        # 페어링 인덱스를 가지고 before pairing의 cost 계산
        src_new_hard, src_new_soft = calculateScore(reshaped_matrix[before_pairing_idx])
        # 페어링 인덱스를 가지고 after pairing의 cost 계산
        trg_new_hard, trg_new_soft = calculateScore(reshaped_matrix[after_pairing_idx])
        # 두개의 pairing에서의 soft값과 hard값을 가지고 현재 cost 계산
    ...
    def reset(self):
        ...
        """
        self.pairing_set = self.pairing_set.flatten()
